 ...
 */
export async function summarizeRiskWithLLM(riskBrief) {
 ...
 */
function generateFallbackSummary(riskBrief) {
 ...
 */
export async function summarizeWeatherWithLLM(weatherData) {
...
Keep it conversational and under 60 words.`;
    try {
        ...
        return `Current conditions at ${wx.airport?.name}: ${wx.current?.condition}. ${wx.hazards?.description !== "none"
            ? `⚠️ Hazards: ${wx.hazards.description}. `
            : "No weather hazards. "}${wx.alerts?.length > 0 ? `Alert: ${wx.alerts[0].event}` : ""}`;
    }
}
/**
