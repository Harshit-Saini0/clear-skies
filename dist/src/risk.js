...
function scoreWeatherFromForecast(forecast, windowHours = 6) {
    try {
        ...
        // Hazard features
        const maxWind = Math.max(...sample.map((h) => h.wind_kph ?? 0), 0);
        const maxGust = Math.max(...sample.map((h) => h.gust_kph ?? 0), 0);
        const minVis = Math.min(...sample.map((h) => h.vis_km ?? 10), 10);
        const thunder = sample.some((h) => String(h.condition?.text || "").toLowerCase().includes("thunder"));
        const snow = sample.some((h) => /(snow|freez|ice|sleet)/i.test(h.condition?.text || ""));
        // Heuristic risk
        let r = 0;
...
function scoreTsa(waitJson, leadMins) {
...
function scoreNews(hits) {
    // Simple keyword escalation: strikes, outage, ATC, weather alerts
    const joined = hits.map(h => h.title.toLowerCase()).join(" | ");
    let r = 0;
    if (/(strike|walkout|industrial action)/i.test(joined))
        r = Math.max(r, 0.7);
    if (/(outage|system failure|it outage|meltdown)/i.test(joined))
        r = Math.max(r, 0.6);
    if (/(atc|air traffic|controller)/i.test(joined))
        r = Math.max(r, 0.5);
    if (/(storm|blizzard|hurricane|typhoon|heatwave)/i.test(joined))
        r = Math.max(r, 0.5);
    const detail = `news: hits=${hits.length}`;
    return { score: r, detail };
}
export async function buildRiskBrief(input) {
    const { flightIata, date, depIata, arrIata, paxType = "domestic", targetArrivalLeadMins } = input;
    ...
    if (f0) {
        ...
        if (/cancel/.test(status))
            opsScore = 1.0;
        else if (/divert/.test(status))
            opsScore = Math.max(opsScore, 0.9);
        else if (/landed|active|scheduled/.test(status))
            opsScore = Math.max(opsScore, 0.1);
    ...
    if (depGeo?.lat && depGeo?.lon) {
        ...
        wxDepScore = s.score;
        wxDepExplain = s.detail.replace(/^wx/, "wx dep");
    }
    if (arrGeo?.lat && arrGeo?.lon) {
        const wx = await getForecast(arrGeo.lat, arrGeo.lon);
        const s = scoreWeatherFromForecast(wx);
        wxArrScore = s.score;
        wxArrExplain = s.detail.replace(/^wx/, "wx arr");
    }
    // TSA
