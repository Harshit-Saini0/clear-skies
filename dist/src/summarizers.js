 ...
 */
export function summarizeFlight(apiResponse) {
    const flight = apiResponse?.data?.[0];
    if (!flight) {
        throw new Error("No flight data found");
    }
    const calcDelay = (scheduled, estimatedOrActual) => {
        if (!scheduled || !estimatedOrActual)
            return undefined;
...
export function summarizeWeather(geo, apiResponse) {
    ...
    const now = Date.now();
    const next6h = hours.filter((h) => {
        const t = new Date(h.time).getTime();
        return t >= now && t <= now + 6 * 3600_000;
    }).slice(0, 6);
    const maxWind = Math.max(...next6h.map((h) => h.wind_kph ?? 0), current.wind_kph ?? 0);
    const maxGust = Math.max(...next6h.map((h) => h.gust_kph ?? 0), current.gust_kph ?? 0);
    const minVis = Math.min(...next6h.map((h) => h.vis_km ?? 10), current.vis_km ?? 10);
    const hasThunder = next6h.some((h) => /thunder|lightning/i.test(h.condition?.text || ""));
    const hasSnow = next6h.some((h) => /snow|ice|sleet|freez/i.test(h.condition?.text || ""));
    const hazards = [];
    if (maxWind > 40 || maxGust > 60)
        hazards.push("high winds");
    if (minVis < 5)
        hazards.push("low visibility");
    if (hasThunder)
        hazards.push("thunderstorms");
    if (hasSnow)
        hazards.push("snow/ice");
...
export function summarizeTsa(iata, apiResponse) {
    ...
    if (latest < 20)
        estimate = "Under 20 minutes";
    else if (latest < 40)
        estimate = "20-40 minutes";
    else if (latest < 60)
        estimate = "40-60 minutes";
    else
        estimate = "Over 1 hour";
    let recommendation = "";
    if (avg < 30)
        recommendation = "Standard arrival buffer is sufficient";
    else if (avg < 45)
        recommendation = "Consider arriving 30 minutes earlier than usual";
    else
        recommendation = "Arrive at least 45-60 minutes earlier; consider TSA PreCheck";
    return {
        airport: iata,
...
export function summarizeNews(query, hits) {
    const riskKeywords = [
